...
# theory: if a game ends super early, the player who loses probably forfeited, admitting to inferiority?
# If it's a tie, then it could mean the game didn't matter to either player, probably means their ratings are closer to each other?
partone <- tapply(abs(train$EloDiff[train$Result=="0-1" | train$Result=="1-0"]), train$GameLength[train$Result=="0-1" | train$Result=="1-0"], mean)
part1.5 <- tapply(abs(train$EloDiff[train$Result=="0-1"]), train$GameLength[train$Result=="0-1"], mean)
part1.75 <-tapply(abs(train$EloDiff[train$Result=="1-0"]), train$GameLength[train$Result=="1-0"], mean)
parttwo <- tapply(abs(train$EloDiff[train$Result=="1/2-1/2"]), train$GameLength[train$Result=="1/2-1/2"], mean)
par(mfcol=c(2,2))
plot(x=as.integer(names(partone)), y=partone, xlim=c(0,325), ylim=c(0,550), xlab="Game Length", ylab="Mean Absolute Elo Difference", main="Mean Absolute Elo Difference by Game Length\nWhen Result is NOT a Tie")
lines(smooth.spline(x=as.integer(names(partone)),y=partone))
p1mv <- movingVariance(partone)
plot(p1mv)
plot(x=as.integer(names(parttwo)), y=parttwo, xlim=c(0,325), ylim=c(0,550), xlab="Game Length", ylab="Mean Absolute Elo Difference", main="Mean Absolute Elo Difference by Game Length\nWhen Result IS a Tie")
lines(smooth.spline(x=as.integer(names(parttwo)),y=parttwo))
p2mv <- movingVariance(parttwo)
plot(p2mv)

# I tried the following, but part1.5 and part1.75 are not different enough to be considered different groups.
# They should be combined in to one group: "Not a tie"
plot(x=as.integer(names(part1.5)), y= part1.5, xlim=c(0,325), ylim=c(0,550), xlab="Game Length", ylab="Mean Absolute Elo Difference", main="Mean Absolute Elo Difference by Game Length\nWhen Result is 0-1")
lines(smooth.spline(x=as.integer(names(part1.5)),y=part1.5))
plot(x=as.integer(names(part1.75)), y= part1.75, xlim=c(0,325), ylim=c(0,550), xlab="Game Length", ylab="Mean Absolute Elo Difference", main="Mean Absolute Elo Difference by Game Length\nWhen Result is 1-0")
lines(smooth.spline(x=as.integer(names(part1.75)),y=part1.75))

# there was definitely a relationship between game length and elo difference. What about between game length and mean elo?
par(mfcol=c(2,3))
part3 <- tapply(train$meanElo, train$GameLength, mean)
plot(x=as.integer(names(part3)), y= part3, xlim=c(0,325), ylim=c(1900,2900), xlab="Game Length", ylab="Mean Elo", main="Mean Elo by Game Length")
lines(smooth.spline(x=as.integer(names(part3)),y=part3))
p3mv <- movingVariance(part3)
plot(p3mv)

part4 <- tapply(train$meanElo[train$Result=='1/2-1/2'], train$GameLength[train$Result=='1/2-1/2'], mean)
plot(x=as.integer(names(part4)), y= part4, xlim=c(0,325), ylim=c(1900,2900), xlab="Game Length", ylab="Mean Elo", main="Mean Elo by Game Length\nWhen Result is a Tie")
lines(smooth.spline(x=as.integer(names(part4 )),y=part4 ))
p4mv <- movingVariance(part4)
plot(p4mv)

part5 <- tapply(train$meanElo[train$Result=='1-0' | train$Result=='0-1'], train$GameLength[train$Result=='1-0' | train$Result=='0-1'], mean)
plot(x=as.integer(names(part5)), y= part5, xlim=c(0,325), ylim=c(1900,2900), xlab="Game Length", ylab="Mean Elo", main="Mean Elo by Game Length\nWhen Result is NOT a Tie")
lines(smooth.spline(x=as.integer(names(part5)),y=part5))
p5mv <- movingVariance(part5)
plot(p5mv)

# It appears that there is a lot of variance in meanElo and EloDiff when GameLength is < about 42 and > 139!
plot(table(train$GameLength))
segments(42,0,42,sum(train$GameLength==42), col="red")
segments(139,0,139,sum(train$GameLength==139), col="red")
# moving variance of partone, parttwo, part4, part5
movingVariance <- function(dat, k=10) {
	k=k-1
	datmv.len <- length(dat)-k
	datmv <- numeric(length=datmv.len)
	for (i in 1:datmv.len) {
		datma <- mean(dat[i:(i+k)])
		datmv[i] <- var(dat[i:(i+k)])
	}
	names(datmv) <- names(dat)[1:datmv.len]
	return(datmv)
}
